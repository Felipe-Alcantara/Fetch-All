...
def analyze_repo(repo: Path, do_fetch: bool = True) -> RepoStatus:
    ...
    try:
        ...
        return RepoStatus(repo, RepoState.GIT_ERROR, detail=str(exc))
...
def _analyze(repo: Path, do_fetch: bool) -> RepoStatus:
    ...
    if git_dir_proc.returncode != 0:
        return RepoStatus(repo, RepoState.GIT_ERROR, detail=git_dir_proc.stderr.strip())
    ...
    if do_fetch:
        ...
        if fetch.returncode != 0:
            return RepoStatus(
                repo, RepoState.FETCH_ERROR, branch=branch,
                detail=fetch.stderr.strip().splitlines()[-1] if fetch.stderr.strip() else "",
            )
    ...
    if upstream.returncode != 0:
        return RepoStatus(
            repo, RepoState.NO_UPSTREAM, branch=branch, dirty_files=dirty_files,
            detail=f"branch '{branch}' não rastreia nenhum branch remoto",
        )

    counts = _git(repo, "rev-list", "--left-right", "--count", "@{u}...HEAD")
    if counts.returncode != 0:
        return RepoStatus(repo, RepoState.GIT_ERROR, branch=branch, detail=counts.stderr.strip())
    behind, ahead = (int(n) for n in counts.stdout.split())

    if dirty_files:
        return RepoStatus(
            repo, RepoState.DIRTY, branch=branch, ahead=ahead, behind=behind,
            dirty_files=dirty_files,
            detail=f"{len(dirty_files)} arquivo(s) modificados/não rastreados",
        )
    if ahead and behind:
        return RepoStatus(
            repo, RepoState.DIVERGED, branch=branch, ahead=ahead, behind=behind,
...
def pull_ff_only(status: RepoStatus) -> tuple[bool, str]:
    ...
    proc = _git(status.path, "pull", "--ff-only")
